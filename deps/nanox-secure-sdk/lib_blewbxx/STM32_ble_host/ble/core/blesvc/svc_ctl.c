...
{
  UNUSED(pfBLE_SVC_Client_Event_Handler);
#if (BLE_CFG_CLT_MAX_NBR_CB > 0)
...
//          BLE_DBG_SVCCTL_MSG("Server SVCCTL_GATT_EVT_TYPE 0x%x\n", blue_evt->ecode);
#if (BLE_CFG_SVC_MAX_NBR_CB > 0)
  ...
  switch (event_notification_status)
  {
    case SVCCTL_EvtNotAck:
       ...
       */
//      BLE_DBG_SVCCTL_MSG("EVT not managed by the service controller (svc_ctl.c) and forwarded to the application\n");
